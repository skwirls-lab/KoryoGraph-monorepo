...
export default async function ApprovalsPage({ searchParams }: { searchParams: Promise<{ kind?: string }> }) {
  ...
  const [{ data: pending }, { data: decided }] = await Promise.all([
    ctx.supabase.from("approval_items").select("id, kind, title, preview, payload, created_at, person_id, people(first_name, last_name, preferred_name), ai_runs(transport)").eq("status", "pending").order("created_at").limit(200),
  ...
  const items: QueueItem[] = (pending ?? []).filter((p) => !kind || p.kind === kind).map((p) => ({
  ...
  return (
    <>
      ...
      <nav aria-label="Filter by kind" className="mb-4 flex flex-wrap gap-1">
        <Button asChild size="sm" variant={!kind ? "secondary" : "ghost"}><Link href="/desk/inbox/approvals" aria-current={!kind ? "page" : undefined}>All ({pending?.length ?? 0})</Link></Button>
