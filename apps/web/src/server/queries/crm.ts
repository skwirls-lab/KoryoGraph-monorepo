...
export async function pipeline(ctx: Ctx): Promise<{ stages: BoardStage[]; leads: BoardLead[]; sessions: TrialSession[] }> {
  ...
  const [{ data: stages }, { data: leads }, { data: programs }, { data: sessions }] = await Promise.all([
    ...
    ctx.supabase.from("leads").select("id, stage_id, source, program_interest, next_action, next_action_at, lost_reason, converted_household_id, stage_changed_at, people(first_name, last_name, preferred_name), bookings(status, class_sessions(name, starts_at))").order("created_at", { ascending: false }).limit(500),
  ...
  return {
    ...
    leads: (leads ?? [])
      ...
      .map((l) => ({
        ...
        nextAction: l.next_action, nextActionDue: l.next_action_at ? l.next_action_at.slice(0, 10) : null,
